...
# `simulate_lump_repayment`: проценты периода больше пересчитанного аннуитета,
# тело зажимается в ноль (calculator.py:310-311), и строка перестаёт сходиться:
# `principal + interest != payment`. Неоплаченные проценты при этом всё равно
# попадают в `total_interest`. Чинится на И1.
KNOWN_UNBALANCED_ROWS = {
    ('lump/long/2000000/between/reduce_payment/adj0', 3): {
        'principal': '0.00', 'interest': '13081.95', 'payment': '7666.34',
    },
    ('lump/long/2000000/between/reduce_payment/adj1', 3): {
        'principal': '0.00', 'interest': '11778.38', 'payment': '7674.36',
    },
}

# `calc_repayment_schedule`: досрочка не всегда получает собственную строку —
# в двух ветках она сливается с аннуитетной, и строка с `early > 0` несёт
# ненулевые проценты. Денежная суть инварианта при этом не нарушена (проценты
# начислены на добытийный остаток, см. test_lump_does_not_reduce_period_interest),
# но форма строки от инварианта отличается. Чинится на И3.
KNOWN_MERGED_EARLY_ROWS_TOTAL = 350        # строк на всей матрице
KNOWN_MERGED_EARLY_ROWS_CASES = 350        # кейсов, в которых они встречаются
...
class KnownDeviationsTest(unittest.TestCase):
    ...
    def test_known_unbalanced_rows_in_lump_simulation(self):
        """
        Известное расхождение, И1: при досрочке 2 000 000 ₽ внутри периода
        пересчитанный аннуитет оказывается меньше процентов периода, тело
        зажимается в ноль (calculator.py:310-311) и строка перестаёт сходиться.
        """
        found = {}
        for case in lump_cases():
